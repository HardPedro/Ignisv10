...
async function startServer() {
  ...
  app.post('/webhooks/zapi', async (req, res) => {
    try {
      ...
      const qNumber = query(numbersRef, where('instanceId', '==', instanceId), where('server_token', '==', 'ignishard18458416'));
      ...
      const qMsg = query(messagesRef, where('wa_message_id', '==', messageId), where('server_token', '==', 'ignishard18458416'));
  ...
  app.post('/api/whatsapp/generate-quote', async (req, res) => {
    try {
      ...
      const waNumberDoc = await getDoc(doc(db, 'whatsapp_numbers', convData.whatsapp_number_id));
      const waNumberData = waNumberDoc.exists() ? waNumberDoc.data() : {};
      ...
      if (mediaUrl) {
        ...
        try {
          ...
          if (mediaResponse.ok) {
            ...
            finalMediaData = `data:${mimeType};base64,${buffer.toString('base64')}`;
          }
        } catch (err) {
          console.error('Failed to download media for Z-API:', err);
        }

        if (mediaType === 'image') {
          endpoint = 'send-image';
          requestBody.image = finalMediaData;
          if (text) requestBody.caption = text;
        } else if (mediaType === 'video') {
          endpoint = 'send-video';
          requestBody.video = finalMediaData;
          if (text) requestBody.caption = text;
        } else if (mediaType === 'audio') {
          endpoint = 'send-audio';
          requestBody.audio = finalMediaData;
        } else if (mediaType === 'document') {
          endpoint = 'send-document';
          requestBody.document = finalMediaData;
