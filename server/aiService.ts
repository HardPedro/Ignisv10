...
export async function handleAIResponse(
  ...
  if (!apiKey) {
    ...
    // Fallback message
    const fallbackMessage = "Desculpe, estou enfrentando instabilidades no momento (API Key não configurada). Um de nossos atendentes falará com você em breve.";
    ...
    try {
      ...
      const res = await fetch(zapiUrl, {
        method: 'POST',
        headers,
        body: JSON.stringify({ phone, message: fallbackMessage })
      });

      if (res.ok) {
        await addDoc(messagesRef, {
          tenantId,
          wa_message_id: `ai_err_${Date.now()}`,
          direction: 'outbound',
          type: 'text',
...
`;
    ...
    // Fallback message
    const fallbackMessage = "Desculpe, estou enfrentando instabilidades no momento. Um de nossos atendentes falará com você em breve.";
